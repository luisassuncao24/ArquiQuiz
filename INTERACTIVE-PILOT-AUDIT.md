...
Phases 4 and 5 convert 14 existing questions into canonical matching or ordering copies for local evaluation. The source records are never mutated. The copies are generated only when `?interactivePreview=1` is used from a local file, `localhost`, or a loopback address.
...
## Candidates deliberately excluded in Phase 5
