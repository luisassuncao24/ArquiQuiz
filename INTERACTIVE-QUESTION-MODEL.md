...
For manual review, `interactive-review.html` is available from the local hub and reads the same migration registry through `createReviewCatalog`. New registered questions from the already loaded exam banks appear there automatically, with no separately maintained ID list. A direct `?question=exam-id` link opens one item; filtering, answer evaluation, reset, and previous-source comparison do not update quiz storage. The page blocks non-local origins. `tests/interactive-review-browser.html` checks this workflow.
