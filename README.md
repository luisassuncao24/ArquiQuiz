...
To inspect migrations without waiting for a shuffled quiz, sign in to the hub and use **Testar perguntas interativas**. The review page is available on the published site as well as locally, using the existing hub session without another password prompt. It lists all entries from the shared migration registry automatically, supports filtering and direct links such as `interactive-review.html?question=pl400-2080`, lets you answer and reset each interaction, and keeps the previous wording behind a comparison panel. It does not write exam progress. As with the rest of this static site, the browser-side hub session is a convenience gate, not server-side access control.
...
The model can be checked with Node (`node tests/question-model.test.cjs`) or directly in a browser with `tests/question-model-browser.html`. The component test is `tests/interactive-question-browser.html`, the end-to-end session test is `tests/app-interactive-integration.html`, the migration-data test is `tests/interactive-pilots-browser.html`, and the public-release, rollback, and saved-result compatibility test is `tests/interactive-pilot-app-browser.html`. The review page has its own browser test at `tests/interactive-review-browser.html`. `tests/interactive-question-demo.html` remains the isolated manual playground. The broader `tests/question-banks-browser.html` page loads the four source banks and verifies that their existing single, multiple, and sequence data remains compatible.
