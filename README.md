...
To inspect migrations without waiting for a shuffled quiz, open `interactive-review.html` locally or use **Rever perguntas interativas** on the local hub. The page lists all entries from the shared migration registry automatically, supports filtering and direct links such as `interactive-review.html?question=pl400-2080`, lets you answer and reset each interaction, and keeps the previous wording behind a comparison panel. It does not write exam progress and blocks itself on non-local hosts.
